...
polygon(x=c(regime[1]-w/2,regime[1]+w/2, regime[1]+w/2,regime[1]-w/2), y=c(rep(-0.5,2),rep(0.5,2)), col=rgb(1,0,0, alpha=.1), border=NA)
polygon(x=c(regime[2]-w/2,regime[2]+w/2, regime[2]+w/2,regime[2]-w/2), y=c(rep(-0.5,2),rep(0.5,2)), col=rgb(1,0,0, alpha=.1), border=NA)

abline(v=c(seq(1980,2010,by=5)), col='grey90')
matlines(yrs[(w-5):(length(yrs)-5)], syg2, type="l", col = c('blue',rep('black',nrow(syg2)-1)), lwd=c(2,rep(1,nrow(syg2)-1)))
lines(yrs[(w-5):(length(yrs)-5)],syg, lwd=2, col='grey80')
