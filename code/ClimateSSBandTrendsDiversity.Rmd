...
One potential message emerging from all of this is that pollock is a strong driver of the icthyoplankton community dynamics, and particularly in the 1980s and 1990s.  After pollock declined in the mid-90s, stronger relationships between ichthyoplankton abundances and climate drivers have begun to emerge.  
...
```
```{r, echo=F}
trends=read.csv("Abund.trends2.csv", header=T)
tr2=trends[,-1]
names(tr2)=c('trend1','trend2')

load=read.csv('Abund.loadings.csv',header=T)
ld=load
names(ld)=c('Species','Trend1','Trend2')

spnames=read.csv("Abund.names.csv", header=T)

climate=read.csv('ClimateCovars.csv',header=T)
SST=read.csv('SSTcovar.csv',header=T)
ssb=read.csv('ssbout.csv', header=T)

###Diversity metrics
occAll=read.csv("occurrenceStatsForDFA.csv", header=T)[,-1]
...
##Synchrony
The last thing I want to look at here are the synchrony indices. These gives us a sense of how similar or different each species is behaving.  I print the code out here since these are custom functions.

```{r}
x <- read.csv("DensityForDFA.csv", stringsAsFactors = FALSE)
...
nsync<-function(x) {
  (1/dim(x)[2])*sum(apply(x,2, function(y) cor(y,rowSums(x, na.rm=T)-y)), na.rm=T)
}


#X=x
w <- 5
sy <- sapply((w+1):nrow(X), function(i) sqrt(synchrony(X[(i - w):i, ])))
wc <- sapply((w+1):nrow(X), function(i) wcv(X[(i - w):i, ]))
wct <- sapply((w+1):nrow(X), function(i)
    sd(rowSums(X[(i - w):i, ], na.rm=T), na.rm=T)/mean(rowSums(X[(i -w):i,], na.rm=T ), na.rm=T))

syg <- sapply((w+1):nrow(X), function(i) nsync(X[(i - w):i, ]))

yrs=seq(1981,2013) #[-c(4,6,32)]
#pdf("syncNoPollock.pdf", width = 5, height = 9)
par(mfrow=c(2,2), mai=c(0.7,0.7,0.2,0.1))
plot(yrs[(1+w):length(yrs)],sy, type="o")
plot(yrs[(1+w):length(yrs)],wc, type="o")
plot(yrs[(1+w):length(yrs)],wct, type="o")
plot(yrs[(1+w):length(yrs)],syg, type="o")
#matplot(yrs[(1+w):length(yrs)], X[6:33,], type = "l", log="")
#dev.off()
```

Next, let's look at the two synchrony metrics with each species jack-knifed out one at a time.  This lets us see how much each species is affecting the synchrony metric. Both plots show the dominant role of pollock, but it influences the first synchrony metric more strongly.  The first metric suggests synchrony is flat or declining, whereas the 2nd shows asynchrony in the early part of the time series, with increasing synchrony during the 90s, that has flattened out, or possibly declined more recently.  The second metric is supposed to be less influenced by uneven communities like this one, so it's probably more reliable.
```{r, echo=F}

sy2 <- sapply(1:ncol(X), function(j)
  sapply((w+1):nrow(X), function(i) sqrt(synchrony(X[(i - w):i, -j]))))
matplot(yrs[(1+w):length(yrs)], sy2, type="l", col = "black")

syg2 <- sapply(1:ncol(X), function(j)
  sapply((w+1):nrow(X), function(i) nsync(X[(i - w):i, -j])))
matplot(yrs[(1+w):length(yrs)], syg2, type="l", col = "black")

```


```{r}
par(mfrow=c(4,1), mar=c(0,4,2,0), oma=c(1,1,1,1))
plot(1981:2013, biod$IchSR, type='o', axes=F, ylab="Species Richness", ylim=c(1.8,11.5))
box()
axis(2, at=seq(0,12,by=2), labels=seq(0,12,by=2), cex.axis=0.8)
par(mar=c(2,4,0,0))
plot(1981:2013, biod$IchSW, type='o', axes=F, ylab="Shannon Diversity", ylim=c(1.8,11.5))
box()
axis(2, at=seq(0,12,by=2), labels=seq(0,12,by=2), cex.axis=0.8)
...
clim.cut=climate[7:39,]
ssb.cut=ssb[ssb$Year>1980 & ssb$Year<2014,]

cor.mat=cbind(clim.cut[,-1],SST[1:33,2:3],ssb.cut[,c(2,3,5)],trend1=tr2[,1],trend2=tr2[,2],biod[,2:3])
...
dens=read.csv('DensityforDFA.csv', header=T)
densc=dens
