...
##
par(mar=c(3,4,0,4))
matplot(yrs[(1+w):length(yrs)], syg2, type="n", col = c('blue',rep('black',nrow(syg2)-1)), 
    lwd=c(2,rep(1,nrow(syg2)-1)), xlim=c(1981,2013), axes=F, ylab="Synchrony", xlab="")
abline(v=c(seq(1980,2010,by=5)), col='grey90')
matlines(yrs[(1+w):length(yrs)], syg2, type="l", col = c('blue',rep('black',nrow(syg2)-1)), lwd=c(2,rep(1,nrow(syg2)-1)))
lines(yrs[(1+w):length(yrs)],syg, lwd=2, col='grey80')
...
cor.mat=cbind(climate[,-2],sst=c(NA,SST[1:32,5]),ssb[,c(2:5)],trend1=tr2[,1],trend2=tr2[,2],biod[,2:3])
