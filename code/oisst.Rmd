...
This is a rough break that likely needs to be further refined

```{r}

outer %>% 
   filter(longitude > -161.1 & longitude < -144.9 & latitude >54 & latitude <61) %>% 
...
SST %>% group_by(year) %>% filter(month > 3 & month < 11) %>% summarize(sst=mean(sst)) -> sSST
...
akmap<-ggplot()+geom_polygon(data=ak,aes(long,lat,group=group),fill=8,color='black')+
   ...
   coord_map(xlim=c(-162,-144),ylim=c(53.5,61.5))
