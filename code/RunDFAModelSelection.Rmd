...
wss <- function(k) {
  kmeans(di, k, nstart = 10 )$tot.withinss
}
...
final<-kmeans(di, 5, nstart=25)


#fviz_nbclust(di, FUN = hcut, method = "wss")



gap_stat <- clusGap(Z.rot[,1:2], FUN = kmeans, nstart = 25,
                    K.max = 15, B = 50)
# Print the result
...
#Z.rot.plot=NA
Z.dat=data.frame(cbind(sp.names, Z.rot,final$cluster))
names(Z.dat)=c('Species', 'LongName','ShortName','Habitat','Guild','Spawn.time','Egg.Ecology','Larval.Duration.wks','Dur.short','Trend1','Trend2','Cluster')
...
Z.rot.plot$Cluster=factor(Z.rot.plot$Cluster)
