...
for(i in 1:length(spp)){
  ...
  box()
  mtext(sp.names$spp[i], side=3, line=-1, cex=0.5)
}
...
The next step is to see if we can improve the model fit by adding explanatory covariates.  We looked at three types of covariates: environmental indices, and index for the EVOS spill year, spawning stock biomasses of pollock, pcod, and arrowtooth. The take-home was that none of these covariates improved the AICc over the two trend model.  We output the code below.

**QUESTION: SHOULD WE LOOK AT OTHER SPECIES SSB AS WELL?? I also tried halibut, but the time series was too short.  My sense is that this is sufficient because nothing we looked at was supported, mostly due to the model complexity I think.**
...
##Best fit DFA
Let's plot the best fit model for each species and calculate the percent variance explained. I propose this as **Figure 2** in the paper.
...
```
Now, we can plot the rotated loadings and trends. I plotted the loadings as a bargraph here, but think they will be better shown in the manuscript as the bivariate plot further down.  The trend plot would be **Figure 3**.
```{r, echo=FALSE}
###################################################
### plotfacloadings
##################################################
...
We can also calculate the proportion of variance explained by the full model, using sum of squares.  The calculated value seems low to me, but I will check into whether it's reasonable.
```{r}
##rsquared
...
di <- dist(Z.no.rot[,1:2], method="euclidean",diag = TRUE, upper = TRUE)


library(rioja)
x2=chclust(di, method = "coniss")
bstick(x2,20)
...
plot(x, labels = sp.names$LongName, hang = 0.1,
     axes = TRUE, frame.plot = FALSE, ann = TRUE,
     main = "",
     sub = NULL, xlab = "", ylab = "Height")
```
Breaking the dendrogram into 4 groups, we can then make a bivariate plot of the loadings, showing the four different groups using different colored text. This plot shows the groupings pretty nicely, so I suggest something like this for **Figure 4**.
```{r, echo=FALSE}
#sp.names$groups=NA
...
ggplot(Z.rot.plot) +
  ...
  geom_point(aes(Trend1,Trend2)) +
  geom_text_repel(aes(Trend1, Trend2), label=row.names(Z.rot.plot))
dev.off()


library(calibrate)
pdf("Loadingsbiplot.pdf", width=6, height=6)
par(mfrow=c(1,1),mar=c(4,4,1,1), oma=c(0,0,0,0))
plot(Z.rot[-no.plot,1],Z.rot[-no.plot,2], ylab="Trend 2", xlab="Trend 1", xlim=c(-1.22,1.22), ylim=c(-.57,.37))
#text(lab.Zrotx,lab.Zroty, sp.names$spp[-no.plot], cex=0.6)
textxy(Z.rot[-no.lab,1],Z.rot[-no.lab,2], sp.names$spp[-no.lab], cex= 0.5, offset=0.55)
text(Z.rot[1,1]-.2,Z.rot[1,2]-.015 , sp.names$spp[1], cex= 0.5)
text(Z.rot[33,1]-.2,Z.rot[33,2] , sp.names$spp[33], cex= 0.5)
text(Z.rot[5,1]+.15,Z.rot[5,2]-.01 , sp.names$spp[5], cex= 0.5)
text(Z.rot[14,1]+.15,Z.rot[14,2]+.012 , sp.names$spp[14], cex= 0.5)
text(Z.rot[31,1]+.2,Z.rot[31,2] , sp.names$spp[31], cex= 0.5)
text(Z.rot[36,1]+.25,Z.rot[36,2]+.012 , sp.names$spp[36], cex= 0.5)
text(Z.rot[9,1]+.12,Z.rot[9,2]+.012 , sp.names$spp[9], cex= 0.5)
text(Z.rot[32,1]-.15,Z.rot[32,2]-.012 , sp.names$spp[32], cex= 0.5)
text(Z.rot[8,1]-.18,Z.rot[8,2]-.014 , sp.names$spp[8], cex= 0.5)
text(Z.rot[4,1]-.23,Z.rot[4,2] , sp.names$spp[4], cex= 0.5)
